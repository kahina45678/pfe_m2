...
def send_question(room_code):
    ...
    # Start a timer for the question using socketio.start_background_task
    def start_timer(room_code, time_limit):
        socketio.sleep(time_limit)  # Attend le temps imparti
...
def handle_create_room(data):
    ...
    user_sessions[user_id] = username  # Ajoute l'hôte à user_sessions

    # Créez la salle sans ajouter l'hôte à la liste des joueurs
    active_rooms[room_code] = {
        'host': username,  # Stocke l'hôte séparément
        'quiz_id': quiz_id,
...
def handle_join_room(data):
    ...
    join_room(room_code)
    user_rooms[user_id] = room_code
    user_sessions[user_id] = username  # Ajoute le joueur à user_sessions

    is_host = active_rooms[room_code]['host'] == username

    # Vérifiez si l'utilisateur est déjà dans la liste des joueurs
    if user_id not in active_rooms[room_code]['players']:
    ...
    print(
        f"[DEBUG] Player {username} ({user_id}) joined room {room_code} - is_host: {is_host}")
    print(f"[DEBUG] Players in room {room_code}: {players_list}")

    emit('room_joined', {'room_code': room_code,
         'is_host': is_host}, to=user_id)
...
def handle_submit_answer(data):
    ...
    print(
        f"[DEBUG] Player {user_id} submitted answer {answer} for question {current_q + 1} in room {room_code}")
    ...
    all_answered = all(current_q in player['answers']
    ...
    # If all answered, move to next question or end game
    if all_answered:
        print(
            f"[DEBUG] All players answered question {current_q + 1} in room {room_code}")
        ...
        if room['current_question'] < len(room['questions']):
            print(
                f"[DEBUG] Sending next question ({room['current_question'] + 1}) to room {room_code}")
            # Attendre 3 secondes avant de passer à la prochaine question
            socketio.sleep(3)
            send_question(room_code)
        else:
            print(f"[DEBUG] Ending game in room {room_code}")
            ...
            # Send final results
            emit('game_over', {'players': players_list,
                 'quiz_id': room['quiz_id']}, to=room_code)
